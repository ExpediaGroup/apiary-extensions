...
- Grouped `apiary-metastore-consumers`, `apiary-metastore-listener` and `apiary-receivers` (all previously submodules in `apiary-metastore-events`) in a new submodule named `sns-metastore-events`, itself a submodule of `apiary-metastore-events`.
